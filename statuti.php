...
<style>
      ...
      <?php
$pdo = new PDO('mysql:host=localhost;dbname=syrine', 'root','');
if (!$pdo) {
  echo "error ";
}

$req = $pdo->query("SELECT role,count(*) as a FROM utilisateur GROUP BY role");
...
const counts = <?php echo json_encode($counts)?>;
  new Chart(ctx, {
    ...
    data: {
      labels: ab ,
      datasets: [{
        label: 'Nombre des utilisateur par role ',
        data: labels,
        borderWidth: 1
      }]
    },
